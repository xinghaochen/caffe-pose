...
def make_output_pose_command(output_pose_bin, model, weights, label_list, output_name, fx, fy, ux, uy):
    ...
    --ux={7} \
    --uy={8} \
    2>&1 | tee logs/test_handpose_baseline_icvl.txt'.format(output_pose_bin, model, weights, label_list, output_name, fx, fy, ux, uy)
    return command
...
solver_name = 'model_icvl/solver_handpose_baseline_icvl.prototxt'
